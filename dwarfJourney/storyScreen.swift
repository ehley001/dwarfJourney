...
class storyScreen: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        appDelegate.story.leftChild = nodeRoot.leftChild
        appDelegate.story.value = nodeRoot.value
        appDelegate.story.rightChild = nodeRoot.rightChild
        
        //let beginning = nodeRoot
        storyText.text = appDelegate.story.value.story
        choiceOne.setTitle(appDelegate.story.value.firstChoiceBtn, for: .normal)
        choiceTwo.setTitle( appDelegate.story.value.secondChoiceBtn, for: .normal)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        else if(appDelegate.story.value.fight)
        {
            appDelegate.story = appDelegate.story.leftChild!
            ...
            if(appDelegate.story.value.secondChoiceBtn == "Start a new Game?")
            {
    ...
    @IBAction func choiceOne(_ sender: UIButton) {
        if(appDelegate.story.value.fight)
        {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "toCombatScreen"?:
            if segue.destination is combatScreen{
                let startPlayer = stats(appDelegate.playerStats.strength,appDelegate.playerStats.health,appDelegate.playerStats.luck)
...
//            appDelegate.value = appDelegate.rightChild!.value
    ...
    lazy var nodeab = Node(nodeaaaa,Story("As you try to climb the wall you find that it is hard to get a good grip. Your hands slip and you fall into the water and get taken away by the current to a shallow point. After getting up you are presented with a rope bridge. What do you do?","Attempt to cross?","Fing another way?",false),nodeaaab)
    lazy var nodeabbbaa = Node(nodeaaaaaaaaa,Story("Fortunately there was enough time to grab you axe and fight the goblin. After killing the goblin you continue on to Mt. Chaos. Treasure awaits on the top of Mt. Chaos. What do you do?","Climb Mt. Chaos?","Find an entrance?",false),nodeaaaaaaaab)
    ...
    lazy var nodeaaaaaaaabbaa = Node(nodeaaaaaaaaba,Story("After defeating the goblin you find that you have reached a dead end.","Go left instead?","",false),nil)
    lazy var nodeabbbbb = Node(nodeaaaaaaaaa,Story("After resting a little you continue onward. You see what looks like a goblin camp where a goblin is sleeping. You quietly go around the camp and make it to the base of Mt. Chaos. Treasure awaits on the top of Mt. Chaos. What do you do?","Climb Mt. Chaos?","Find an entrance?",false),nodeaaaaaaaab)
    
    // Intermediate Redirection Nodes
    lazy var nodeaaaaaaaabb = Node(nodeaaaaaaaabba,Story("As you take the right path you find that it twists and turn taking you deep into the mountain. The path opens up into a wide cavern. You can see something shining on the other side. What do you do?","Follow the light?","Go left instead?",false),nodeaaaaaaaaba)
    ...
    lazy var nodebaa = Node(nodebaaa,Story("Going through the left tunnel the smell gets stronger. You then find yourself in a room with another dwarf baking blueberry pie what do you do?","Talk to the dwarf?","Go right instead?",false),nodebb)
    ...
    lazy var nodeaaaaaaaa = Node(nodeaaaaaaaaa,Story("After resting you continue on through the forest till you find yourself outside of the forest and in front of Mt. Chaos. Treasure awaits on the top of Mt. Chaos. What do you do?","Climb Mt. Chaos?","Find an entrance?",false),nodeaaaaaaaab)
    ...
    lazy var nodeaaa = Node(nodeaaaa,Story("You jump into the water and the current quickly pulls you under. Finally getting to a shallow point you are able to get out of the water and continue on your journey. While walking you come across a rope bridge. What do you do?","Attempt to cross?","Find another way?",false),nodeaaab)
